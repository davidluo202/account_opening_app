...
export default function RiskQuestionnaire() {
  ...
  useEffect(() => {
    if (savedData) {
      setFormData({
        ...
        q2_investment_period: savedData.q2_investment_period ? JSON.parse(savedData.q2_investment_period) : [],
        q3_price_volatility: savedData.q3_price_volatility ? JSON.parse(savedData.q3_price_volatility) : [],
        q4_investment_percentage: savedData.q4_investment_percentage ? JSON.parse(savedData.q4_investment_percentage) : [],
        q5_investment_attitude: savedData.q5_investment_attitude ? JSON.parse(savedData.q5_investment_attitude) : [],
        q6_derivatives_knowledge: savedData.q6_derivatives_knowledge ? JSON.parse(savedData.q6_derivatives_knowledge) : [],
        q7_age_group: savedData.q7_age_group ? JSON.parse(savedData.q7_age_group) : [],
        q8_education_level: savedData.q8_education_level ? JSON.parse(savedData.q8_education_level) : [],
        ...
        q10_liquidity_needs: savedData.q10_liquidity_needs ? JSON.parse(savedData.q10_liquidity_needs) : [],
  ...
  const calculateScore = (): { totalScore: number; riskLevel: string } => {
    ...
    if (formData.q2_investment_period.includes("less_than_1")) score += 10;
    if (formData.q2_investment_period.includes("1_to_3")) score += 30;
    if (formData.q2_investment_period.includes("more_than_3")) score += 50;

    // Q3: 可以接受的年度價格波幅（A=10分，B=30分，C=50分）
    if (formData.q3_price_volatility.includes("10_percent")) score += 10;
    if (formData.q3_price_volatility.includes("20_percent")) score += 30;
    if (formData.q3_price_volatility.includes("30_percent")) score += 50;

    // Q4: 資產淨值中可作投資用途的百分比（A=10分，B=20分，C=30分，D=40分，E=50分）
    if (formData.q4_investment_percentage.includes("less_than_10")) score += 10;
    if (formData.q4_investment_percentage.includes("10_to_20")) score += 20;
    if (formData.q4_investment_percentage.includes("21_to_30")) score += 30;
    if (formData.q4_investment_percentage.includes("31_to_50")) score += 40;
    if (formData.q4_investment_percentage.includes("more_than_50")) score += 50;

    // Q5: 對金融投資的一般態度（A=10分，B=20分，C=30分，D=40分，E=50分）
    if (formData.q5_investment_attitude.includes("no_volatility")) score += 10;
    if (formData.q5_investment_attitude.includes("small_volatility")) score += 20;
    if (formData.q5_investment_attitude.includes("some_volatility")) score += 30;
    if (formData.q5_investment_attitude.includes("large_volatility")) score += 40;
    if (formData.q5_investment_attitude.includes("any_volatility")) score += 50;
    ...
    if (formData.q7_age_group.includes("18_to_25")) score += 20;
    if (formData.q7_age_group.includes("26_to_35")) score += 30;
    if (formData.q7_age_group.includes("36_to_50")) score += 40;
    if (formData.q7_age_group.includes("51_to_64")) score += 20;
    if (formData.q7_age_group.includes("65_plus")) score += 10;

    // Q8: 教育程度（A=10分，B=30分，C=50分）
    if (formData.q8_education_level.includes("primary_or_below")) score += 10;
    if (formData.q8_education_level.includes("secondary")) score += 30;
    if (formData.q8_education_level.includes("tertiary_or_above")) score += 50;
    ...
    if (formData.q10_liquidity_needs.includes("no_need")) score += 50;
    if (formData.q10_liquidity_needs.includes("up_to_30")) score += 30;
    if (formData.q10_liquidity_needs.includes("30_to_50")) score += 20;
    if (formData.q10_liquidity_needs.includes("over_50")) score += 10;

    // 判定風險等級（根據總分直接對應風險水平）
    let riskLevel = "";
    if (score <= 99) riskLevel = "Lowest / 最低";
    else if (score <= 199) riskLevel = "Low / 低";
    else if (score <= 299) riskLevel = "Low to Medium / 低至中等";
    else if (score <= 399) riskLevel = "Medium / 中等";
    else if (score <= 599) riskLevel = "Medium to High / 中等至高";
    else riskLevel = "High / 高";
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.q2_investment_period.length === 0) newErrors.q2 = "請至少選擇一項";
    if (formData.q3_price_volatility.length === 0) newErrors.q3 = "請至少選擇一項";
    if (formData.q4_investment_percentage.length === 0) newErrors.q4 = "請至少選擇一項";
    if (formData.q5_investment_attitude.length === 0) newErrors.q5 = "請至少選擇一項";
    if (formData.q6_derivatives_knowledge.length === 0) newErrors.q6 = "請至少選擇一項";
    if (formData.q7_age_group.length === 0) newErrors.q7 = "請至少選擇一項";
    if (formData.q8_education_level.length === 0) newErrors.q8 = "請至少選擇一項";
    if (formData.q9_investment_knowledge_sources.length === 0) newErrors.q9 = "請至少選擇一項";
    if (formData.q10_liquidity_needs.length === 0) newErrors.q10 = "請至少選擇一項";
  ...
  const handleSubmit = () => {
    ...
    saveMutation.mutate({
      ...
      q2_investment_period: JSON.stringify(formData.q2_investment_period),
      q3_price_volatility: JSON.stringify(formData.q3_price_volatility),
      q4_investment_percentage: JSON.stringify(formData.q4_investment_percentage),
      q5_investment_attitude: JSON.stringify(formData.q5_investment_attitude),
      q6_derivatives_knowledge: JSON.stringify(formData.q6_derivatives_knowledge),
      q7_age_group: JSON.stringify(formData.q7_age_group),
      q8_education_level: JSON.stringify(formData.q8_education_level),
      q9_investment_knowledge_sources: JSON.stringify(formData.q9_investment_knowledge_sources),
      q10_liquidity_needs: JSON.stringify(formData.q10_liquidity_needs),
      totalScore,
      riskLevel,
    });
  };
