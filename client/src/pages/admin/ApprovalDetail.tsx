...
export default function ApprovalDetail() {
  ...
  // 获取制裁筛查结果
  const { data: sanctionsData } = trpc.sanctions.getResult.useQuery(
    { applicationId: Number(id) },
    { enabled: !!id }
  );
      ...
      <div className="container mx-auto py-8">
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              申请编号：{application?.applicationNumber} | 系统编码：{application?.applicationCode || '-'} | 提交时间：{application?.submittedAt ? new Date(application.submittedAt).toLocaleString("zh-HK") : "-"}
            </CardDescription>
          </CardHeader>
        </Card>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              制裁名单筛查 (AML/KYC)
              {sanctionsData ? (
                sanctionsData.hitCount > 0 ? (
                  <span className="px-2 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">命中 {sanctionsData.hitCount} 条</span>
                ) : (
          ...
          {sanctionsData && (
            <CardContent>
              <div className="text-sm space-y-2">
                <div><span className="text-muted-foreground">筛查姓名：</span>{sanctionsData.screenedName}</div>
                <div><span className="text-muted-foreground">筛查ID：</span><span className="font-mono text-xs">{sanctionsData.searchId}</span></div>
                <div><span className="text-muted-foreground">筛查时间：</span>{sanctionsData.createdAt ? new Date(sanctionsData.createdAt).toLocaleString("zh-HK") : "-"}</div>
                {sanctionsData.hitCount > 0 && sanctionsData.hits && (
                  <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-md">
                    <p className="font-semibold text-red-800 mb-2">命中详情：</p>
                    <pre className="text-xs text-red-700 whitespace-pre-wrap overflow-auto max-h-40">{typeof sanctionsData.hits === 'string' ? sanctionsData.hits : JSON.stringify(sanctionsData.hits, null, 2)}</pre>
                  </div>
                )}
                {sanctionsData.flaggedForReview && (
                  <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-md text-amber-800 text-sm font-medium">
                    ⚠ 此申请已标记需人工复核
                  </div>
                )}
              </div>
